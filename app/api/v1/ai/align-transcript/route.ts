...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!rawText || typeof rawText !== 'string') {
      return NextResponse.json(
        { error: 'Invalid request', message: 'rawText is required' },
        { status: 400 }
      )
    }
    ...
    if (!duration || typeof duration !== 'string') {
      return NextResponse.json(
        { error: 'Invalid request', message: 'duration is required' },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error ? error.message : 'Failed to align transcript',
      },
      { status: 500 }
    )
  }
