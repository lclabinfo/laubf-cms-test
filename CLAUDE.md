...
- `lib/tenant/context.ts` — `getChurchId()` for website routes (reads `x-tenant-id` header, falls back to env var)
...
- `app/website/` — Public website routes (layout + catch-all page; proxy rewrites `laubf.lclab.io/about` → `/website/about` internally)
...
- `laubf-test/` — Original public website (separate Next.js app, migration source — being retired in favor of root `app/website/` routes)
...
- `00_old_laubf_db_dump/` — SQL dump files from the legacy LA UBF website database (MySQL). Contains ~120 tables covering messages, bible studies, events, announcements, prayer, member boards, and more. Used as source data for migrating legacy content into the new Prisma/PostgreSQL schema. See `docs/old-db-migration/` for analysis and migration plans.
...
**Two-app structure (transitional):** Root project = CMS admin + API + public website (via `app/website/` routes). `laubf-test/` = original public website (separate Next.js app, now superseded by root project's `app/website/` — retained as reference but being retired). The `proxy.ts` rewrites subdomain requests (e.g., `laubf.lclab.io/about` → `/website/about`) so the `/website` prefix never appears in the browser URL.
...
**CRITICAL — Always reuse existing project components instead of native HTML elements.** Before adding any form field or interactive element, check `components/ui/` for an existing implementation. Common mistakes to avoid:
- **Date inputs:** Use `<DatePicker>` (`components/ui/date-picker.tsx`), never `<input type="date">`.
- **Tooltips:** Use `<Tooltip>` / `<TooltipProvider>` / `<TooltipTrigger>` / `<TooltipContent>` (`components/ui/tooltip.tsx`), never the native `title` attribute for interactive hints.
- **Number inputs:** Use `<Input type="number">` with spinner-hiding classes (`[appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none`) to match the design system. The native browser stepper arrows do not match the project's visual style.
- **Selects:** Use `<Select>` from `components/ui/select.tsx`, never `<select>`.
- **Switches/toggles:** Use `<Switch>` from `components/ui/switch.tsx`, never `<input type="checkbox">` styled as toggle.
- **Dialogs/modals:** Use `<Dialog>` from `components/ui/dialog.tsx`, never custom modal implementations.
- **Popovers:** Use `<Popover>` from `components/ui/popover.tsx` for interactive overlays.

When in doubt, search `components/ui/` and `components/cms/` for how similar patterns are already implemented in the codebase.
...
| **User Journeys** | `docs/08_user-journeys/` | Auth edge cases, onboarding flows, QA checklists, security vulnerability docs, member management flows |
...
- **Website Phase B.1: COMPLETE** — `app/website/` routes, section registry (42 entries), ThemeProvider, FontLoader, navbar, footer, SectionWrapper, catch-all page route, tenant context, design system CSS ported.
