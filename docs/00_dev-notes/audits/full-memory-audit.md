...
|----------|--------|--------|---------|
| 1 | Add ISR (`export const revalidate`) to website routes | 15 min | 100-300 MB under traffic |
| 2 | Reduce members context from pageSize=500 to 50 + server pagination | 2-3 hrs | 2-5 MB per CMS session |
| 3 | Add `take` limit to `getRecurringEvents()` | 5 min | Prevents unbounded growth |
| 4 | Add `pageSize` max guard on people API | 5 min | Prevents 10K+ record requests |
| 5 | Replace rate limiter Map with LRU cache | 30 min | Prevents ~2-3 MB leak |
| 6 | Unify builder iframe data (postMessage instead of re-fetch) | 2-3 hrs | ~330 KB per builder session |
| 7 | Add `description` to Event global omit | 10 min | ~5-50 KB per event list |
| 8 | Cap builder undo history at 10 entries | 15 min | Prevents 2.5 MB+ growth |
| 9 | Batch R2 bulk deletes to 10-20 concurrent | 30 min | Prevents spike during bulk ops |
| 10 | Add bulk operation array length validation | 10 min | Prevents DoS via large arrays |
