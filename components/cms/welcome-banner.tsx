...
export function WelcomeBanner({ userName, userId, churchName, roleName }: WelcomeBannerProps) {
  // Start dismissed to match server render (avoids hydration mismatch)
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      if (params.get("dev-welcome") === "true") {
        setDismissed(false)
        return
      }
    }
    const wasDismissed = !!localStorage.getItem(`cms-welcome-dismissed-${userId}`)
  ...
  return (
    <div className="relative mt-5 rounded-xl border-0 px-6 py-5 bg-gradient-to-r from-primary/10 via-primary/5 to-transparent">
      <Button
        variant="ghost"
        size="icon"
