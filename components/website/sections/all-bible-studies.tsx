import { getBibleStudies, getBibleStudyFilterMeta, type BibleStudyFilterMeta } from '@/lib/dal/bible-studies'
...
interface Props {
  ...
  filterMeta?: BibleStudyFilterMeta
}
...
export default async function AllBibleStudiesSection({ content, churchId, studies: preloadedStudies, pagination: preloadedPagination, filterMeta: preloadedFilterMeta }: Props) {
  try {
    ...
    let filterMeta = preloadedFilterMeta
    if (!studies && churchId) {
      const PAGE_SIZE = 50
    ...
    return <AllBibleStudiesClient studies={studies ?? []} heading={content.heading} pagination={pagination} filterMeta={filterMeta} />
  } catch (error) {
